...
const startStats = html.indexOf('<!-- Statistics -->');
const endStats = html.indexOf('</div>\r\n      </div>', startStats);
if (startStats !== -1 && endStats !== -1) {
  ...
  html = html.substring(0, startStats) + ghostHTML + html.substring(endStats);
} else {
  console.log("Failed to find Statistics panel");
}

// 6. Replace Top Spending with Active Subscriptions
const startSpending = html.indexOf('<div class="spending-row">');
const endSpending = html.indexOf('</div>\r\n      </div>\r\n\r\n    </div>', startSpending);
if (startSpending !== -1 && endSpending !== -1) {
  const activeSubsHTML = `<div class="spending-row">
  ...
  html = html.substring(0, startSpending) + activeSubsHTML + html.substring(endSpending);
} else {
  console.log("Failed to find Spending Row");
}
