...
// Target directory
const p2Dir = path.join(__dirname, '../project2-coliving-manager');

// 1. Delete redundant files
const filesToDelete = ['dashboard.html', 'ledger.html', 'history.html', 'data.js'];
filesToDelete.forEach(file => {
    try {
        fs.unlinkSync(path.join(p2Dir, file));
...
// 2. Generate new HTML wrapper (index.html)
const htmlContent = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>CoSpace | Co-living Manager</title>
...
<div class="portfolio-demo-bar">
  <div class="demo-info">
    ...
    <span class="demo-text">Interactive Prototype — SPA Gated Logic</span>
  </div>
  <a href="../index.html#project-02" class="back-portfolio-btn">
    <i class="ph-bold ph-arrow-left"></i> Back to Portfolio
  </a>
</div>
...
<div class="page-container">
  <header class="unified-header">
    <div class="top-nav">
      <div class="nav-brand">
        <div class="nav-brand-icon"><i class="ph-bold ph-house-line"></i></div>
        <div class="nav-brand-name">CoSpace</div>
      </div>
      <div class="nav-links">
        <a href="#" class="nav-link active" data-target="kanban">Kanban</a>
        <a href="#" class="nav-link" data-target="ledger">Split Ledger</a>
        <a href="#" class="nav-link" data-target="settle">Settle Up</a>
      </div>
      <div class="nav-right">
        <button class="nav-icon-btn"><i class="ph-bold ph-bell"></i></button>
...
fs.writeFileSync(path.join(p2Dir, 'index.html'), htmlContent, 'utf8');

// 3. Generate app.js logic
const jsContent = `
...
fs.writeFileSync(path.join(p2Dir, 'app.js'), jsContent, 'utf8');

// Append CSS modifications to styles.css
const cssMod = `
/* --- SPA INJECTED STYLES --- */
.empty-state { padding: 40px; text-align: center; color: var(--text2); background: var(--bg-card); border-radius: 12px; border: 1px dashed var(--border); margin-top: 24px; font-weight: 600; }
.badge-overdue { background: #fee2e2; color: #ef4444; border: 1px solid #fecaca; }
.card-overdue { border: 2px solid #ef4444 !important; background: #fff5f5 !important; }
.btn-confirm { background: #10B981; color: white; border: none; }
.btn-confirm:hover { background: #059669; }
.modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: none; align-items: center; justify-content: center; z-index: 1000; }
.modal-overlay.active { display: flex; }
.modal-card { background: var(--bg-card); width: 90%; max-width: 400px; padding: 24px; border-radius: 12px; position: relative; box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
.modal-close { position: absolute; top: 12px; right: 12px; background: transparent; border: none; font-size: 20px; cursor: pointer; color: var(--text2); }
.modal-close:hover { color: var(--text); }
`;
fs.appendFileSync(path.join(p2Dir, 'styles.css'), cssMod, 'utf8');
