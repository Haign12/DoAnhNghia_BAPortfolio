...
<div class="portfolio-demo-bar">
  ...
  <main class="main-content">
    ...
    <div id="view-home" class="view-section active fade-in">
      ...
      <div class="card-list" id="activeSessionsList">
        <!-- Injected via JS -->
      </div>

      <div class="section-title mt-5">Recent Orders</div>
      <div class="card-list" id="recentOrdersList">
    ...
    <div id="view-create" class="view-section fade-in">
      ...
      <div class="form-card">
        <div class="form-group">
          <label>Shop / Restaurant Name <span style="color:red">*</span></label>
          <input type="text" class="form-input" id="createShopName" placeholder="e.g. Highlands Coffee, Phuc Long...">
        </div>
        <div class="form-group">
          <label>Menu Link (Optional)</label>
          <input type="text" class="form-input" id="createMenuLink" placeholder="Paste ShopeeFood / GrabFood link">
        </div>
        <div class="form-row">
          <div class="form-group" style="flex: 1;">
            <label>Cut-off Date <span style="color:red">*</span></label>
            <input type="date" class="form-input" id="createCutoffDate">
          </div>
          <div class="form-group" style="flex: 1;">
            <label>Cut-off Time <span style="color:red">*</span></label>
            <input type="time" class="form-input" id="createCutoffTime">
          </div>
        </div>
        <div class="form-group">
          <label>Shipping Fee Split Method <span style="color:red">*</span></label>
          <select class="form-input" id="createSplitMethod">
            <option>Equal split among all members</option>
            <option>Proportional to order amount</option>
          </select>
        </div>
        <div class="form-actions mt-4">
          <button class="btn-primary w-100" onclick="handleCreateOrder()">Create & Generate Invite Link</button>
        </div>
      </div>
    </div>
    ...
    <div id="view-host-dashboard" class="view-section fade-in">
      ...
      <div class="page-header" style="display: flex; justify-content: space-between; align-items: flex-end;">
        <div>
          <h1 class="page-title" id="hostDashShopName">Highlands Coffee</h1>
          <div class="page-subtitle">Session <strong id="hostDashSessionCode">HC8273</strong> • Closes at <span id="hostDashCutoff">11:30 AM</span></div>
        </div>
        <div class="share-box">
          <input type="text" id="hostDashInviteLink" value="https://orderflow.app/join/HC8273" readonly class="share-input">
      ...
      <div class="stepper-section" style="padding: 24px;">
        <div style="display: flex; justify-content: space-between; margin-bottom: 12px;">
          ...
          <div style="font-weight: 700; color: var(--teal); font-size: 1.1rem;" id="fundingProgressText">0 / 0 Paid</div>
        </div>
        <div style="width: 100%; height: 8px; background: var(--border-light); border-radius: 4px; overflow: hidden; margin-bottom: 8px;">
          <div id="fundingProgressBar" style="width: 0%; height: 100%; background: var(--teal); border-radius: 4px; transition: width 0.3s ease;"></div>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-secondary);" id="fundingProgressDesc">Waiting for participants to join.</div>
      </div>
        ...
        <div class="dashboard-main">
          <div class="card">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
              ...
              <button class="btn-secondary" style="padding: 4px 12px; font-size: 12px;" onclick="simulateCutoff()">Simulate Cut-off Trigger</button>
            </div>
        ...
        <div class="dashboard-side">
          <div class="card">
            ...
            <div class="summary-row">
              <span>Subtotal</span>
              <span id="hostSubtotal">0đ</span>
            </div>
            <div class="summary-row">
              <span>Shipping Fee</span>
              <span id="hostShipping">0đ</span>
            </div>
            <div class="summary-row">
              <span>Discount</span>
              <span style="color: var(--green);" id="hostDiscount">0đ</span>
            </div>
            <div class="summary-divider"></div>
            <div class="summary-total">
              <span>Grand Total</span>
              <span class="total-amount" id="hostTotal">0đ</span>
            </div>

            <div class="mt-4">
              <button class="btn-primary w-100 mb-2" id="btnSendToVendor" onclick="switchView('view-vendor-summary')" disabled>Close Order & Send to Vendor</button>
              <button class="btn-secondary w-100" onclick="handlePreviewParticipant()">Preview as Participant</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- VIEW: SESSION CLOSED -->
    <div id="view-session-closed" class="view-section fade-in">
      <button class="back-link" onclick="switchView('view-home')"><i class="ph ph-arrow-left"></i> Home</button>
      <div class="payment-container text-center" style="padding: 80px 40px;">
        <i class="ph ph-lock-key" style="font-size: 4rem; color: var(--border-medium); margin-bottom: 20px;"></i>
        <h2 class="mb-2">Session Closed</h2>
        <p class="text-muted mb-4">This order session has passed its cut-off time and is no longer accepting new items.</p>
        <button class="btn-primary" onclick="switchView('view-home')">Go to My Orders</button>
      </div>
    </div>
    ...
    <div id="view-participant" class="view-section fade-in">
      <div class="page-header centered text-center">
        <div class="host-avatar" style="width: 48px; height: 48px; border-radius: 50%; background: var(--bg-dark); color: #fff; display: flex; align-items: center; justify-content: center; font-weight: bold; margin: 0 auto 12px; font-size: 1.2rem;">DN</div>
        <h1 class="page-title" style="font-size: 1.5rem;">Join order from <span id="participantShopName">Shop Name</span></h1>
          ...
          <div class="card" style="position: sticky; top: 100px;">
            ...
            <div id="cartFilled" style="display: none;">
              ...
              <div class="summary-row" style="font-size: 13px;">
                ...
                <span id="cartSharedFee">+ 0đ</span>
              </div>
              <div class="summary-total mt-2" style="font-size: 1.1rem;">
                <span>Your Total</span>
                <span id="cartTotal">0đ</span>
              </div>
              <button class="btn-primary w-100 mt-3" id="checkoutBtn" onclick="proceedToPayment()" disabled>Confirm & Pay</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="payment-container text-center">
        ...
        <div class="qr-card">
          ...
          <div class="qr-instruction" style="background: rgba(0,0,0,0.05); padding: 12px; border-radius: 8px; font-family: monospace;">
            Message: <strong style="color: var(--text-primary);" id="paymentTransferMessage">CODE-Name</strong>
          </div>
        </div>

        <button class="btn-primary w-100 mt-4" onclick="switchView('view-payment-pending')">I have transferred</button>
        <button class="btn-text w-100 mt-2" onclick="switchView('view-participant')">Back to Cart</button>
      </div>
    </div>

    <!-- VIEW: PAYMENT PENDING CONFIRMATION -->
    <div id="view-payment-pending" class="view-section fade-in">
      <div class="payment-container text-center" style="padding: 60px 40px;">
        <i class="ph ph-hourglass-high" style="font-size: 4rem; color: var(--orange); margin-bottom: 20px;"></i>
        <h2 class="mb-2">Awaiting Confirmation</h2>
        <p class="text-muted mb-4">Your payment status is pending. Please wait for the Host or system webhook to verify the transfer.</p>
        <div style="background: var(--bg-main); padding: 16px; border-radius: 12px; text-align: left; margin-bottom: 24px;">
          <div class="summary-row" style="font-size: 13px;"><span>Order:</span> <strong id="pendingShopName">Shop Name</strong></div>
          <div class="summary-row" style="font-size: 13px;"><span>Amount:</span> <strong id="pendingAmountDisplay">0đ</strong></div>
          <div class="summary-row" style="font-size: 13px;"><span>Status:</span> <strong style="color: var(--orange);">PENDING CONFIRMATION</strong></div>
        </div>
        ...
        <div style="margin-top: 16px; font-size: 12px; color: var(--text-secondary);">
          Demo Note: In a real app, this screen auto-updates via WebSocket when the Host clicks 'Confirm' or bank webhook triggers.
        </div>
      </div>
    </div>
      ...
      <div class="payment-container" style="max-width: 600px; padding: 40px;">
        <div style="display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2px dashed var(--border-medium); padding-bottom: 24px; margin-bottom: 24px;">
          <div>
            ...
            <p class="text-muted" id="vendorShopName">Shop Name</p>
          </div>
        ...
        <button class="btn-primary w-100 mt-4" onclick="markOrderCompleted()">Mark as Vendor Received & Completed</button>
      </div>
    </div>
...
console.log('Project 3 updated with Gates!');
